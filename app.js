...
function getOrderedEditors() {
  ...
  var saved = COL_ORDER.filter(function (e) {
    return S.editors.indexOf(e) !== -1;
  });
  var newOnes = S.editors.filter(function (e) {
    return saved.indexOf(e) === -1;
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".fp-t-pill").forEach(function (btn) {
    btn.addEventListener("click", function () {
      document.querySelectorAll(".fp-t-pill").forEach(function (b) {
        b.classList.remove("active");
      });
      btn.classList.add("active");
  ...
  ["fStatus", "fCategory", "fListType", "fPhotographer", "fBeds"].forEach(
    function (id) {
  ...
  document
    .getElementById("getExtensionBtn")
    .addEventListener("click", function () {
    ...
    .then(function (json) {
      ...
      var newEditors = Object.keys(newData)
        .filter(function (name) {
          return EXCLUDED_TABS.indexOf(name) === -1;
        })
        .sort(function (a, b) {
      ...
      if (silent) {
        ...
        if (!changed) {
          newEditors.forEach(function (e) {
            if ((newData[e] || []).length !== (S.data[e] || []).length)
              changed = true;
          });
        }
        if (!changed) {
...
function restoreUIState(state) {
  requestAnimationFrame(function () {
    var board = document.getElementById("boardView");
    if (board && state.boardScrollLeft)
    ...
    var tableWrap = document.getElementById("tableView");
    if (tableWrap && state.tableScrollTop)
...
function getRows(editor) {
  var base =
    editor === "all"
      ? getAllRows()
      : (S.data[editor] || []).map(function (r) {
          return Object.assign({ _editor: editor }, r);
        });
  ...
  return base.filter(function (r) {
    ...
    if (S.filters.category && r["Category"] !== S.filters.category)
      return false;
    if (S.filters.listType && r["List Type"] !== S.filters.listType)
      return false;
    if (S.filters.photographer && r["Photographer"] !== S.filters.photographer)
      return false;
...
function rowMatch(r) {
  ...
  return [
    ...
    return r[f] && String(r[f]).toLowerCase().includes(s);
  });
}
...
function renderStats(rows) {
  ...
  var uploaded = rows.filter(function (r) {
    return norm(r["Status"]) === "uploaded";
  }).length;
  // Fix: match both 'ongoing' (old) and 'pending' (new) status values
  var pending = rows.filter(function (r) {
    return norm(r["Status"]) === "pending" || norm(r["Status"]) === "ongoing";
  }).length;
  var rejected = rows.filter(function (r) {
    return norm(r["Status"]) === "rejected";
  }).length;
...
function renderBoard(rows) {
  ...
  if (S.editor === "all") {
    ...
    ordered.forEach(function (editor) {
      var edRows = rows.filter(function (r) {
        return r._editor === editor;
      });
    ...
    statuses.forEach(function (status) {
      var sRows = rows.filter(function (r) {
...
function makeColumn(title, rows) {
  ...
  if (rows.length === 0) {
    ...
    sortNewest(rows).forEach(function (row) {
      body.appendChild(makeCard(row));
    });
  }
...
function makeCard(row) {
  ...
  card.innerHTML =
    ...
    '<div class="card-req">' +
    esc(row["DP-REQ Number"] || "—") +
    "</div>" +
    (listType
      ? '<span class="card-listtype ' +
        listTypeCls +
        '">' +
        esc(listType) +
        "</span>"
...
function renderTable(rows) {
  ...
  rows.forEach(function (row) {
    ...
    allCols.forEach(function (col) {
      if (col === "Editor") {
        html +=
          '<td><span class="sbadge s-default">' +
          esc(row._editor || "") +
          "</span></td>";
        ...
        html +=
          "<td>" +
          (v ? '<a href="' + esc(v) + '" target="_blank">↗ View</a>' : "") +
          "</td>";
      } else if (col === "Status") {
        var v = row[col] || "";
        html +=
        ...
        html +=
          '<td class="tbl-mono" style="color:var(--green)">' +
          esc(String(row[col] || "")) +
          "</td>";
      } else {
...
function openModal(row) {
  ...
  var html =
    ...
    "<div>" +
    '<div class="modal-req">' +
    esc(row["DP-REQ Number"] || "Listing Detail") +
    "</div>" +
    '<div class="modal-ref">' +
    esc(row["Listing Reference"] || "") +
    "</div>" +
    '<div style="margin-top:6px"><span class="sbadge ' +
    sCls +
    '">' +
    esc(status || "—") +
    "</span></div>" +
  ...
  if (row["Listing Link"]) {
    html +=
      ...
      '<div class="d-val"><a href="' +
      esc(row["Listing Link"]) +
      '" target="_blank">↗ Open Listing</a></div>' +
      "</div>";
  }
  ...
  html +=
    ...
    '<div class="d-val">' +
    esc(fmtDateFull(row["Date Uploaded"])) +
    "</div>" +
    "</div>";
  ...
  if (row._editor) {
    html +=
      '<div class="modal-editor">Editor tab: ' + esc(row._editor) + "</div>";
  }
...
function openEditModal() {
  ...
  var editFields = [
    ...
    { key: "Photographer", type: "text" },
    {
      key: "List Type",
      type: "select",
      options: ["", "Photo Request", "Agent Request", "Brochure"],
    },
  ...
  var formRows = editFields
    .map(function (f) {
      var val = String(row[f.key] || "");
      var isFull =
        f.type === "textarea" || f.key === "Listing Link" ? " full" : "";
      var input = "";
      ...
      if (f.type === "select") {
        ...
        input =
          '<textarea class="edit-input" data-field="' +
          esc(f.key) +
          '" rows="3">' +
          esc(val) +
          "</textarea>";
        ...
        input =
          '<input class="edit-input" type="text" data-field="' +
          esc(f.key) +
          '" value="' +
          esc(val) +
          '">';
  ...
  var html =
    ...
    "<div>" +
    '<div class="modal-req">' +
    esc(row["DP-REQ Number"] || "Edit Listing") +
    "</div>" +
    '<div class="modal-ref">' +
    esc(row["Listing Reference"] || "") +
    "</div>" +
    ...
    'Received Date: <strong style="color:var(--text2)">' +
    esc(fmtDate(row["Received Date"])) +
    "</strong> (unchanged)" +
    "</div>" +
      ...
      navigator.clipboard
        ...
        .then(function (text) {
          ...
          try {
            ...
            alert(
              'Clipboard does not contain valid copied data. Use the "📋 Copy Data" button in the extension first.',
            );
            return;
          }
          ...
          if (!parsed.__dp_edit_paste__) {
            alert(
              'Clipboard data was not copied from the DP extension. Use "📋 Copy Data" button first.',
            );
            return;
          }
          ...
          var filled = 0;
          document
            .querySelectorAll("#modalInner .edit-input")
        ...
        .catch(function () {
          alert(
            'Could not read clipboard. Make sure you clicked "📋 Copy Data" in the extension and allow clipboard access.',
          );
        });
    });
...
function getIncomingForRange() {
  ...
  return inRange.reduce(function (acc, r) {
    return {
      "Photo Request":
        (acc["Photo Request"] || 0) + (parseInt(r["Photo Request"], 10) || 0),
      "Agent Request":
        (acc["Agent Request"] || 0) + (parseInt(r["Agent Request"], 10) || 0),
...
function renderReport() {
  ...
  var actualPhoto = allRows.filter(function (r) {
    return norm(r["List Type"]) === "photo request";
  }).length;
  var actualAgent = allRows.filter(function (r) {
    return norm(r["List Type"]) === "agent request";
  }).length;
  var actualBroch = allRows.filter(function (r) {
    return norm(r["List Type"]) === "brochure";
  }).length;
  var actualTotal = actualPhoto + actualAgent + actualBroch;

  var uploaded = allRows.filter(function (r) {
    return norm(r["Status"]) === "uploaded";
  }).length;
  ...
  var compRate =
    allRows.length > 0 ? Math.round((uploaded / allRows.length) * 100) : 0;

  var lifestyleRows = (S.data["Lifestyle"] || []).filter(rowInRange);
  var actualLifestyle = lifestyleRows.reduce(function (s, r) {
    return s + (parseInt(r["Lifestyle"], 10) || 0);
  }, 0);
  var actualProfile = lifestyleRows.reduce(function (s, r) {
    return s + (parseInt(r["Profile"], 10) || 0);
  }, 0);
  var actualOthers = lifestyleRows.reduce(function (s, r) {
    return (
      s + (parseInt(r["Others"], 10) || 0) + (parseInt(r["Count"], 10) || 0)
    );
  }, 0);
  ...
  function diffBadge(actual, expected) {
    ...
    if (diff > 0)
      return '<span class="inc-diff inc-over">▲ +' + diff + "</span>";
  ...
  function incCard(label, actual, expected, valClass) {
    return (
      ...
      "</div>" +
      (expected !== null
        ? '<div class="inc-expected">Expected: <strong>' +
          expected +
          "</strong>" +
          diffBadge(actual, expected) +
          "</div>"
  ...
  var editorBreakdown = S.editors
    ...
    .sort(function (a, b) {
      return (
        ...
        b.profile +
        b.others -
        (a.total + a.lifestyle + a.profile + a.others)
      );
    });

  var team = editorBreakdown.reduce(
    function (s, r) {
      return {
  ...
  var editorRows = editorBreakdown
    .map(function (r) {
      return (
        "<tr>" +
        ...
        num(r.agent) +
        num(r.broch) +
        num(r.lifestyle, "var(--purple)") +
        num(r.profile, "var(--blue)") +
        num(r.others, "var(--orange)") +
        num(r.total) +
        "</tr>"
      );
    })
  ...
  wrap.innerHTML =
    ...
    (expTotal !==
      null +
        ...
        expTotal +
        "</strong>" +
        diffBadge(actualTotal, expTotal) +
        "</div>" +
        '<div class="inc-expected inc-no-data">No morning input</div>') +
    "</div>" +
    "</div>" +
    ...
    num(team.agent) +
    num(team.broch) +
    num(team.lifestyle, "var(--purple)") +
    num(team.profile, "var(--blue)") +
    num(team.others, "var(--orange)") +
    num(team.total) +
    "</tr>" +
    '<tr class="pending-row"><td>Pending</td><td colspan="6"></td><td class="num-cell">' +
    pending +
    "</td></tr>" +
    '<tr class="rate-row"><td>Completion Rate</td><td colspan="6"><span style="font-size:11px;color:var(--text3)">Uploaded ÷ Total</span></td><td class="num-cell">' +
    compRate +
    "%</td></tr>" +
    "</tbody></table>" +
    "</div>";
...
function openIncomingModal() {
  ...
  var fields = [
    {
      key: "Photo Request",
      label: "📷 Photographer Photos",
      color: "var(--blue)",
    },
    {
      key: "Agent Request",
      label: "🏠 Agent Property Photos",
      color: "var(--orange)",
    },
  ...
  var inputsHtml = fields
    .map(function (f) {
      ...
      return (
        '<div class="detail-item">' +
        '<div class="d-label" style="color:' +
        f.color +
        '">' +
        f.label +
        "</div>" +
        '<input class="edit-input incoming-field" type="number" min="0" step="1" data-field="' +
        esc(f.key) +
        '" value="' +
        val +
        '" placeholder="0">' +
  ...
  document.getElementById("incomingCloseBtn").onclick = function () {
    overlay.remove();
  };
  document.getElementById("incomingCancelBtn").onclick = function () {
    overlay.remove();
  };
  document.getElementById("incomingSaveBtn").onclick = function () {
    saveIncoming(overlay);
  };
}
...
function saveIncoming(overlay) {
  ...
  fetch(S.url, {
    ...
    .then(function (json) {
      ...
      if (existIdx >= 0) {
        S.data["Incoming"][existIdx] = newRow;
      } else {
        S.data["Incoming"].push(newRow);
      }
      overlay.remove();
      renderReport();
    })
    ...
    .then(function (json) {
      ...
      if (row._editor && S.data[row._editor]) {
        S.data[row._editor] = S.data[row._editor].filter(function (r) {
          if (row._rowIndex && row._rowIndex > 0)
            return r._rowIndex !== row._rowIndex;
          return !(
            r["DP-REQ Number"] === row["DP-REQ Number"] &&
...
function openAmenitiesModal() {
  ...
  document.getElementById("amenitiesSubtitle").textContent =
    "Search properties for amenity info";
...
function toggleAmenitiesMode() {
  ...
  document.getElementById("amenitiesSearchMode").style.display = isAdd
    ? "none"
    : "";
  document.getElementById("amenitiesAddMode").style.display = isAdd
    ? ""
    : "none";
  document.getElementById("amenitiesToggleBtn").textContent = isAdd
    ? "🔍 Search"
    : "➕ Add New";
  document.getElementById("amenitiesSubtitle").textContent = isAdd
    ? "Add a new amenity entry"
    : "Search properties for amenity info";
  ...
  if (isAdd) {
    ...
    setTimeout(function () {
      document.getElementById("amLocation").focus();
    }, 100);
  } else {
    setTimeout(function () {
      document.getElementById("amenitiesSearchInput").focus();
...
function renderAmenitiesResults(query) {
  ...
  if (amenities.length === 0) {
    wrap.innerHTML =
      '<div class="am-empty">No amenities added yet.<br>Click ➕ Add New to get started.</div>';
    return;
  }

  if (filtered.length === 0) {
    wrap.innerHTML =
      '<div class="am-empty">No results for "<strong>' +
      esc(query) +
      '</strong>"</div>';
    return;
  }
  ...
  var cardsHtml = filtered
    .map(function (r) {
      ...
      return (
        '<a class="am-result-card" href="' +
        esc(link) +
        '" target="_blank" rel="noopener">' +
        '<div class="am-result-loc">🏢 ' +
        loc +
        "</div>" +
